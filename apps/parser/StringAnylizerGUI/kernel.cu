...
void ProcessSequences(const string& fileName, const bool saveBadLines, LegacyBuffers& buffers, vector<Cell> & cellHash, vector<string> & junkLines, vector<string> & badLines, StatisticsInfo & statisticsInfo, vector<string> & SC_stats, string ENTRY, string & whereatCU, string & whereatCount)
{
	...
	while (!infile.eof())
	{
		...
		if (pos == string::npos && line.size() > 0){ // Not header line
			sequence += line;
			continue;
		}
...
__global__ void string_match_kernel(char * d_sequenceBuffer, int * d_sequenceLength, int sequenceCounter, 	int validSCNumber, 
...
void workOutGPU(const bool saveBadLines, LegacyBuffers& buffers, vector<Cell> & cellHash, vector<string> & junkLines, vector<string> & badLines)
{
	...
	for (int i = 0; i < buffers.sequenceCounter; i++) // Go through all the sequences (on this part)
	{
		...
		if (detectInfo.errorCount == 0) // If sequence is valid
		{
			try
			{
				Cell tempCell(barcode, ahca, random, removeMe);
	...
	TemplateLenInfo templateLenInfo, DetectionInfo * d_detectionInfo, char * d_resultBuffer, int AllowedEditNums, int * d_SCx_stat)
{
	...
	__shared__ int DIST[BLOCK_SIZE][VALID_AHCA_LEN + 1][VALID_AHCA_LEN + 1]; // This use for Levenshtein both Barcode and AHCA
...
#pragma region SC3, Random Code and reverse complement as needed
		...
		while (i < sequenceLength - templateLenInfo.SC3 && !found)
		{
			int offset = 0;
			int mutationNum = 0;
			while (mutationNum <= 1 && offset < templateLenInfo.SC3)
			{
				if (sequence[i + offset] != cuda_constants::SC3[offset])
				{
					mutationNum++;
			...
			// Check if SC3 found at current offset i
			if (offset == templateLenInfo.SC3 && mutationNum <= 1)
			{
				mutationRate += mutationNum;
				totalDNARead += offset;
				found = true;
					...
					else // Use Levenshein Distance fix
					{
						...
						while (BarCodeValid != BARCODE_VALID && scId < validSCNumber)
						{
							...
							for (unsigned int ii = 1; ii <= VALID_SC_LEN; ++ii){
								...
								{
									DIST[tid][ii][jj] = min(min(DIST[tid][ii - 1][jj] + 1, DIST[tid][ii][jj - 1] + 1),
										DIST[tid][ii - 1][jj - 1] + (sequence[startOffset + ii - 1] == SC[jj - 1] ? 0 : 1));
								}
							}
							if (DIST[tid][VALID_SC_LEN][VALID_SC_LEN] <= AllowedEditNums)
							{
								BarCodeValid = BARCODE_VALID;
								for (int kk = 0; kk < VALID_SC_LEN; kk++) // fix the sequence
								{
...
#pragma region AHCA
		...
		while (i <= sequenceLength - templateLenInfo.Anneal_Pre && !foundPre_Anneal)
		{
			...
			if (offset == templateLenInfo.Anneal_Pre && mutationNum <= 1 && (i - 1 + templateLenInfo.Anneal_Pre + 9) <= sequenceLength)
			{
				mutationRate += mutationNum;
				totalDNARead += offset;
		...
		if (foundPre_Anneal)
		{
			...
			while (startOffset < MAX_SEQUENCE_LEN)
			{
					...
					else if (AHCAType == 0)                    // Use Levenshtein Distance fix only on Anti (type A)
					{
						...
						while (AHCAValid != AHCA_VALID && ahcaId < validAHCANumber)
						{
							...
							for (unsigned int ii = 1; ii <= VALID_AHCACODE_LEN; ++ii){
								...
								{
									DIST[tid][ii][jj] = min(min(DIST[tid][ii - 1][jj] + 1, DIST[tid][ii][jj - 1] + 1),
										DIST[tid][ii - 1][jj - 1] + (sequence[startOffset + ii - 1] == cuda_constants::SC[jj - 1] ? 0 : 1));
								}
							}

							if (DIST[tid][VALID_AHCACODE_LEN][VALID_AHCACODE_LEN] < AHCADistance) {
								AHCADistance = DIST[tid][VALID_AHCACODE_LEN][VALID_AHCACODE_LEN];
							}

							if (DIST[tid][VALID_AHCACODE_LEN][VALID_AHCACODE_LEN] <= AHCACodemutation)//AllowedEditNums)    //???
							{
								AHCAValid = AHCA_VALID;
								//printf("3->%s, %d, %d\n", AHCA, len, AHCACodemutation);
...
__global__ void RandomCode_Similarity_kernel(char * d_RandomCodeBuffer, int * d_RandomCodeMatchingTable, int RandomCodeSize, int i)
{
...
int GetRandomCodeNumber(LegacyBuffers& buffers, char * const h_RandomCodeBuffer, const int RandomCodeSize)
{
	int number = 0;
